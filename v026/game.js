...
(() => {
  ...
  // Gaps pulled in a touch (55 / 75 / 90 = 220 000 u total) with the same
  // flat 60 s per stage — sustained ~1300 u/s (~81 % MAX_SPEED) now finishes
  // on the wire, so it's challenging but no longer brutal.
  const STAGES = [
    { distance: 55000,  duration: 60 },
    { distance: 130000, duration: 60 },
    { distance: 220000, duration: 60 },  // = chequered flag
  ];
  ...
  // fades out while growing past the camera. Race timer + checkpoints
  // are paused for the duration so the player gets to read it.
  const TRACK_INTRO_DURATION = 2.4;     // total — snappy on a mobile race
  const TRACK_INTRO_HOLD     = 1.3;     // hold time before the fade-out
  ...
  function render(dt) {
    ...
    if (!explosion.active) {
      ...
      // ── Active-shield aura: two counter-spinning magenta hex rings wrapped
      // around the car so the powered-up state is unmistakable. In the final
      // stretch it flashes on/off — the blink accelerates toward zero so you
      // get a clear "about to run out" warning before the shield drops.
      if (boosting) {
        const FLASH_AT = 2.0;               // seconds left when the warning starts
        const frac     = Math.max(0, car.boostT / FLASH_AT);
        const expiring = car.boostT < FLASH_AT;
        // Square-wave strobe whose rate climbs from ~3.5 Hz to ~9 Hz as the
        // timer drains — the quickening flash reads as "running out".
        const blinkHz  = 3.5 + (1 - frac) * 5.5;
        const on       = !expiring || (Math.floor(phase * blinkHz) % 2 === 0);
        if (on) {
          // On the strobe's "on" beat each flash pops: white-hot, thicker, and
          // a touch larger than the steady magenta ring, so it can't be missed.
          const flash   = expiring ? 1 : 0;
          const col     = flash ? "#ffffff" : "#ff5cff";
          const pulse   = 0.5 + 0.5 * Math.sin(phase * 8);
          const ringRot = phase * 1.2;      // matches the pickup-hex spin rate
          const baseR   = 30 + pulse * 4 + flash * 6; // breathing (car body ≈ 22)
          for (let ring = 0; ring < 2; ring++) {
            const dir = ring === 0 ? 1 : -1; // counter-rotate the two rings
            const rr2 = baseR + ring * 8;
            const w   = (ring === 0 ? 2.8 : 1.5) + pulse * 0.9 + flash * 1.4;
            let prev = null;
            for (let k = 0; k <= 6; k++) {
              const a  = dir * ringRot + k * (Math.PI / 3) + ring * 0.26;
              const pt = p(Math.cos(a) * rr2, Math.sin(a) * rr2);
              if (prev) beam(prev.x, prev.y, pt.x, pt.y, col, w, G_FX);
              prev = pt;
            }
          }
          // Bright phosphor sparks at the inner hex vertices.
          for (let k = 0; k < 6; k++) {
            const a  = ringRot + k * (Math.PI / 3);
            const pt = p(Math.cos(a) * baseR, Math.sin(a) * baseR);
            dots.push({ x: pt.x, y: pt.y, c: "#ffffff",
                        i: 1.0 + pulse * 0.6 + flash * 0.9, g: G_FX });
          }
        }
      }
    }
  ...
  function drawTrackIntro() {
    ...
    // Compute a base name-size that fits 88% of the viewport, then apply
    // the animation scale on top. Long titles like "TURQUOISE HEXAGON SUN"
    // shrink to fit on narrow phones; on desktop they sit at the ideal 84 px.
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    const idealName = 84;
    const maxWidth  = W * 0.88;
    ctx.font = `${idealName}px "Press Start 2P", monospace`;
    const measured  = ctx.measureText(race.trackName).width;
    const fit       = measured > maxWidth ? maxWidth / measured : 1;
    const baseName  = idealName * fit;
    const baseNum   = baseName * 0.33;

    const nameSize  = baseName * scale;
    const numSize   = baseNum  * scale;

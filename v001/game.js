...
(() => {
  ...
  const TRACK_HW = 95;        // half-width
  const SEG_LEN  = 28;        // distance between centerline points
  const track    = [];        // { s, cx, cy, dir, lx, ly, rx, ry }
  ...
  function ensureTrackTo(s) {
    while (track.length === 0 || track[track.length - 1].s < s) {
      ...
      const k1 = (noise1(sNew * 0.0009) - 0.5) * 0.10;
      const k2 = (noise1(sNew * 0.0042) - 0.5) * 0.025;
      ...
      track.push({
        s: sNew, cx, cy, dir,
        lx: cx + nx * TRACK_HW, ly: cy + ny * TRACK_HW,
        rx: cx - nx * TRACK_HW, ry: cy - ny * TRACK_HW,
      });
    }
  }
  ...
  function chunkScenery(idx) {
    const out = [];
    const seed = idx * 1013904223 + 1664525;
    for (let i = 0; i < 7; i++) {
      ...
      const offset = TRACK_HW + 60 + r3 * 380;
  ...
  const TURN_RATE   = 1.4;   // peak yaw rate (rad/s) at low speed
  const OFFROAD_DRAG = 1.6;  // multiplier on coast when off track
  ...
  function updateCar(dt) {
    ...
    const off = Math.abs(info.lateral) - TRACK_HW;
    if (off > 0) {
      const drag = COAST * OFFROAD_DRAG * Math.min(1, off / 50);
      car.speed -= drag * dt;
      car.rumble = Math.min(1, off / 60);
    } else {
      car.rumble *= 0.85;
    }
  ...
  function render(dt) {
    ...
    const rj = car.rumble ? (Math.random() - 0.5) * car.rumble * 3 : 0;
    function p(x, y) {
      const cs = Math.cos(tilt), sn = Math.sin(tilt);
      return { x: cx0 + (x * cs - y * sn) + rj, y: cy0 + (x * sn + y * cs) };
    }

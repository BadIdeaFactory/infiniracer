...
(() => {
  ...
  // Stages are now ~10× longer — full run is 240 000 world units. Each
  // stage gives the player a flat 90 seconds; any time left when you hit
  // a checkpoint carries over and is added to the next stage's clock.
  const STAGES = [
    { distance: 60000,  duration: 90 },
    { distance: 140000, duration: 90 },
    { distance: 240000, duration: 90 },  // = chequered flag
  ];
  const TIME_BONUS_PER_SEC = 250000;
  const FINISH_OVERLAY_DURATION = 6.0;  // seconds before auto-reset
  ...
  function updateCar(dt) {
    ...
    // Post-finish: hold for FINISH_OVERLAY_DURATION (~6s) showing the
    // result screen, then auto-reset to stage 1. Physics still run during
    // the hold so the car coasts to a halt naturally.
    if (race.finished) {
      race.finishedT += dt;
  ...
  function drawFinishOverlay() {
    ...
    const a = Math.min(1, t / 0.4) * (1 - Math.max(0, (t - (FINISH_OVERLAY_DURATION - 0.6)) / 0.6));
  ...
  function updateHUD() {
    ...
    // Score display priorities: explosion → frozen explosion score;
    // race finished → locked final score; otherwise live (speed × dist).
    let score;
    if (explosion.active)      score = explosion.finalScore;
    else if (race.finished)    score = race.finalScore;
    else                       score = Math.floor(car.speed * dst);
